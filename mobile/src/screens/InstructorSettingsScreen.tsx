...
type Props = BottomTabScreenProps<InstructorTabParamList, "InstructorSettings">;
...
export default function InstructorSettingsScreen(_props: Props) {
  ...
  const [lname, setLname] = useState(String(instructorData?.lname ?? ""));
  const [email, setEmail] = useState(String(instructorData?.email ?? ""));
  const [password, setPassword] = useState(String(instructorData?.password ?? ""));
  ...
  async function save() {
    ...
    setLoading(true);
    try {
      const res = await fetchResponse(instructorEndpoints.editInstructor(instructorId), 2, body);
      if (!res?.success) {
        toastError(res?.message ?? "Update failed");
        return;
      }
      toastSuccess(res.message ?? "Updated");
      setInstructorData({ ...(instructorData ?? {}), ...body } as UserRecord);
    } finally {
      setLoading(false);
    }
  }
